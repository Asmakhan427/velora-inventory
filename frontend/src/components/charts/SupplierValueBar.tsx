...
interface SupplierValueBarProps {
  data: { supplier: string; product_count: number; stock_value: number }[]
}

export function SupplierValueBar({ data }: SupplierValueBarProps) {
  if (data.length === 0) {
    return <div className="flex h-64 items-center justify-center text-sm text-text-muted">No supplier data yet.</div>
  }
        ...
        <BarChart data={data} layout="vertical" margin={{ left: 0, right: 24, top: 4, bottom: 4 }} barCategoryGap={14}>
          ...
          <Bar dataKey="stock_value" radius={[0, 4, 4, 0]} maxBarSize={22} isAnimationActive animationDuration={700}>
            {data.map((entry, index) => (
              <Cell key={entry.supplier} fill={CATEGORICAL_PALETTE[index % CATEGORICAL_PALETTE.length]} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
